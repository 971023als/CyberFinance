...
try {
    ...
    if ($antivirusSignatureAge -le 1) {
        ...
        Add-Content -Path $global:TMP1 -Value "WARN: Windows Defender의 바이러스 정의가 최신 상태가 아닙니다. (업데이트된 지 $antivirusSignatureAge 일)"
    }
} catch {
    Add-Content -Path $global:TMP1 -Value "WARN: Windows Defender의 업데이트 상태를 확인하는 데 실패했습니다."
}
