...
try {
    ...
    if ($auditPolicy.CrashOnAuditFail -eq 2) {
        Add-Content -Path $global:TMP1 -Value "OK: 보안 감사 실패 시 시스템이 즉시 종료되도록 설정됨"
    } else {
        Add-Content -Path $global:TMP1 -Value "WARN: 보안 감사 실패 시 시스템이 즉시 종료되지 않도록 설정됨"
    }
} catch {
    Add-Content -Path $global:TMP1 -Value "WARN: 보안 감사 정책 설정을 검사하는데 실패함"
}
